	...
	while (L2 > 0.0) { /* If L2 <= 0, the random walk is over*/
		...
		if (sqrt(fb_sqr(w[0]+vcm[1])+fb_sqr(w[1]+vcm[2])) <= vlc) { 
			...
			if(star[index].binind > 0 && WRITE_BH_LOSSCONE_INFO){ //Binary
				...
				parafprintf(bhlossconefile, "%g 0 %g %ld -100 %g -100 %g -100 %g -100 %ld -100 -100 -100 %g %g %g %g %g %g %g\n", TotalTime, star[index].r, star[index].id, star[index].m * units.mstar / MSUN, star[index].rad  * units.l / RSUN, star[index].se_rc * units.l / RSUN, star[index].se_k, star[index].r_peri * units.l / AU, star[index].r_apo * units.l / AU, v[1], v[2], v[3], star[index].E + MPI_PHI_S(star_r[g_index], g_index), star[index].J );
